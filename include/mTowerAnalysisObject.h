...
#ifndef _MTOWERANALYSISOBJECT_H_
#define _MTOWERANALYSISOBJECT_H_
...
class mTowerAnalysisObject
{
  ...
  private:
    ...
    // mTower geometry
    mTowerGeometry fGeometry; // geometry object in which number of columns, rows etc. is stored as well as conversion tables for lane/chip/layer numbers
  ...
  public:
    ...
    mTowerAnalysisObject(); // default constructor
    mTowerAnalysisObject(const int &run); // constructor initializing variables depending on run number
    ~mTowerAnalysisObject(); // default destructor
    //~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
    ...
    mTowerGeometry GetGeometry ()                   { return fGeometry; };
    void           SetGeometry (mTowerGeometry geo) { fGeometry = geo; };
...
template<class T> T* mTowerAnalysisObject::InitHist2DChip (string name)
{
