...
mTowerGeometry::mTowerGeometry (eDTP period) :
fPeriod(period)
{
  init();
};
...
mTowerGeometry::mTowerGeometry (unsigned int run) :
...
// useful functions
void mTowerGeometry::PrintDetectorGeometry ()
{
  pair<int, int> chips;
  pair<int, int> lanes;
...
void mTowerGeometry::init ()
{
  ...
  else if (fPeriod == kNov19)
  {
    ...
    std::cout << " ### mTower geometry for november data taking not yet implemented. " << std::endl;
  }

  else
  {
    std::cout << " ### Undefined data taking period, cannot initialize mTower geometry properly. " << std::endl;
  }
};
